...
class acp_garage_fields
{
	...
	function main($id, $mode)
	{
		...
		switch ($action)
		{
			case 'delete':
				...
				if (confirm_box(true))
				{
					$sql = 'SELECT field_ident 
						FROM ' . GARAGE_CUSTOM_FIELDS_TABLE . " 
					...
					$db->sql_query('DELETE FROM ' . GARAGE_CUSTOM_FIELDS_TABLE . " WHERE field_id = $field_id");
					$db->sql_query('DELETE FROM ' . GARAGE_CUSTOM_FIELDS_LANG_TABLE . " WHERE field_id = $field_id");
					$db->sql_query('DELETE FROM ' . GARAGE_FIELDS_LANG_TABLE . " WHERE field_id = $field_id");
					...
					while ($row = $db->sql_fetchrow($result))
					{
						...
						{
							$sql = 'UPDATE ' . GARAGE_CUSTOM_FIELDS_TABLE . " 
			...
			case 'edit':
				...
				if ($action == 'edit')
				{
					...
					$sql = 'SELECT l.*, f.*
						FROM ' . GARAGE_FIELDS_LANG_TABLE . ' l, ' . GARAGE_CUSTOM_FIELDS_TABLE . ' f 
					...
					$sql = 'SELECT *
						FROM ' . GARAGE_CUSTOM_FIELDS_LANG_TABLE . ' 
				...
				$gf->vars['field_ident']		= request_var('field_ident', $field_row['field_ident']);
				$gf->vars['lang_name']			= request_var('lang_name', $field_row['lang_name'], true);
				$gf->vars['lang_explain']		= request_var('lang_explain', $field_row['lang_explain'], true);
				$gf->vars['lang_default_value']	= request_var('lang_default_value', $field_row['lang_default_value'], true);
				...
				if (isset($_REQUEST['field_option']))
				{
					...
					$gf->vars['field_required'] = ($field_option == 'field_required') ? 1 : 0;
					$gf->vars['field_show_on_reg'] = ($field_option == 'field_show_on_reg') ? 1 : 0;
					$gf->vars['field_hide'] = ($field_option == 'field_hide') ? 1 : 0;
				}
				else
				{
					$gf->vars['field_required'] = $field_row['field_required'];
					$gf->vars['field_show_on_reg'] = $field_row['field_show_on_reg'];
					$gf->vars['field_hide'] = $field_row['field_hide'];
				...
				$gf->vars['field_no_view'] = request_var('field_no_view', $field_row['field_no_view']);
				...
				if ($options)
				{
					...
					if (sizeof($exploded_options) == sizeof($lang_options) || $action == 'create')
					{
						...
						$gf->vars['lang_options'] = $exploded_options;
					}
					else if ($action == 'edit')
					{
				...
				{
					$gf->vars['lang_options'] = $lang_options;
				}
				...
				foreach ($exclude[2] as $key)
				{
					...
					if ($field_type == FIELD_TEXT && $key == 'field_length')
					{
						...
						{
							$gf->vars['rows'] = request_var('rows', 0);
							$gf->vars['columns'] = request_var('columns', 0);
							$var = $gf->vars['rows'] . '|' . $gf->vars['columns'];
						}
						else
						{
							$row_col = explode('|', $var);
							$gf->vars['rows'] = $row_col[0];
							$gf->vars['columns'] = $row_col[1];
						}
					}
					...
					if ($field_type == FIELD_DATE && $key == 'field_default_value')
					{
						...
						if ($always_now || $var == 'now')
						{
							...
							$gf->vars['field_default_value_day'] = $now['mday'];
							$gf->vars['field_default_value_month'] = $now['mon'];
							$gf->vars['field_default_value_year'] = $now['year'];
						...
						else
						{
							...
							{
								$gf->vars['field_default_value_day'] = request_var('field_default_value_day', 0);
								$gf->vars['field_default_value_month'] = request_var('field_default_value_month', 0);
								$gf->vars['field_default_value_year'] = request_var('field_default_value_year', 0);
								$var = $_POST['field_default_value'] = sprintf('%2d-%2d-%4d', $gf->vars['field_default_value_day'], $gf->vars['field_default_value_month'], $gf->vars['field_default_value_year']);
							}
							else
							{
								list($gf->vars['field_default_value_day'], $gf->vars['field_default_value_month'], $gf->vars['field_default_value_year']) = explode('-', $var);
							}
						}	
					}
				...
				{
					$gf->vars[$key] = request_var($key, array(0 => ''), true);

					if (!$gf->vars[$key] && $action == 'edit')
					{
						$gf->vars[$key] = $$key;
					}
					else if ($key == 'l_lang_options' && sizeof($gf->vars[$key]) > 1)
					{
						foreach ($gf->vars[$key] as $lang_id => $options)
						{
							$gf->vars[$key][$lang_id] = explode("\n", $options);
						}
					}
				}
				...
				if ($submit) //  && $step == 1
				{
					...
					if ($gf->vars['field_ident'] == '')
					{
						$error[] = $user->lang['EMPTY_FIELD_IDENT'];
					}

					if (!preg_match('/^[a-z_]+$/', $gf->vars['field_ident']))
					{
						$error[] = $user->lang['INVALID_CHARS_FIELD_IDENT'];
					}

					if (strlen($gf->vars['field_ident']) > 17)
					{
						$error[] = $user->lang['INVALID_FIELD_IDENT_LEN'];
					}

					if ($gf->vars['lang_name'] == '')
					{
						$error[] = $user->lang['EMPTY_USER_FIELD_NAME'];
					}

					if ($field_type == FIELD_DROPDOWN && !sizeof($gf->vars['lang_options']))
					{
						$error[] = $user->lang['NO_FIELD_ENTRIES'];
					}

					if ($field_type == FIELD_BOOL && (empty($gf->vars['lang_options'][0]) || empty($gf->vars['lang_options'][1])))
					{
						$error[] = $user->lang['NO_FIELD_ENTRIES'];
					}
					...
					if ($action != 'edit')
					{
						$sql = 'SELECT field_ident 
							FROM ' . GARAGE_CUSTOM_FIELDS_TABLE . " 
							WHERE field_ident = '" . $db->sql_escape($gf->vars['field_ident']) . "'";
						$result = $db->sql_query($sql);
						$row = $db->sql_fetchrow($result);
						$db->sql_freeresult($result);
				...
				if (!sizeof($error))
				{
					...
					{
						$this->save_profile_field($gf, $field_type, $action);
					}
				...
				switch ($step)
				{
					...
					case 1: 
						...
						$template->assign_vars(array(
							'S_STEP_ONE'		=> true,
							'S_FIELD_REQUIRED'	=> ($gf->vars['field_required']) ? true : false,
							'S_SHOW_ON_REG'		=> ($gf->vars['field_show_on_reg']) ? true : false,
							'S_FIELD_HIDE'		=> ($gf->vars['field_hide']) ? true : false,
							'S_FIELD_NO_VIEW'	=> ($gf->vars['field_no_view']) ? true : false,

							'L_LANG_SPECIFIC'	=> sprintf($user->lang['LANG_SPECIFIC_OPTIONS'], $config['default_lang']),
							'FIELD_TYPE'		=> $user->lang['FIELD_' . strtoupper($gf->profile_types[$field_type])],
							'FIELD_IDENT'		=> $gf->vars['field_ident'],
							'LANG_NAME'			=> $gf->vars['lang_name'],
							'LANG_EXPLAIN'		=> $gf->vars['lang_explain'])
						);
						...
						if ($field_type == FIELD_STRING || $field_type == FIELD_TEXT)
						{
							$template->assign_vars(array(
								...
								'L_DEFAULT_VALUE_EXPLAIN'	=> $user->lang[strtoupper($gf->profile_types[$field_type]) . '_DEFAULT_VALUE_EXPLAIN'],
								'LANG_DEFAULT_VALUE'		=> $gf->vars['lang_default_value'])
							);
						}
						...
						if ($field_type == FIELD_BOOL || $field_type == FIELD_DROPDOWN)
						{
							...
							if (!sizeof($gf->vars['lang_options']))
							{
								if ($field_type == FIELD_BOOL)
								{
									// No options have been defined for a boolean field.
									$gf->vars['lang_options'][0] = '';
									$gf->vars['lang_options'][1] = '';
								}
								else
								{
									// No options have been defined for the dropdown menu
									$gf->vars['lang_options'] = array();
								}
							}
							...
							$template->assign_vars(array(
								...
								'L_LANG_OPTIONS_EXPLAIN'	=> $user->lang[strtoupper($gf->profile_types[$field_type]) . '_ENTRIES_EXPLAIN'],
								'LANG_OPTIONS'				=> ($field_type == FIELD_DROPDOWN) ? implode("\n", $gf->vars['lang_options']) : '',
								'FIRST_LANG_OPTION'			=> ($field_type == FIELD_BOOL) ? $gf->vars['lang_options'][0] : '',
								'SECOND_LANG_OPTION'		=> ($field_type == FIELD_BOOL) ? $gf->vars['lang_options'][1] : '')
							);
						}
					...
					case 2:
						...
						$function = 'get_' . $gf->profile_types[$field_type] . '_options';
						$options = $gf->$function();
					...
					case 3: 
						...
						$options = $this->build_language_options($gf, $field_type, $action);
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$template->assign_block_vars('fields', array(
				...
				'FIELD_TYPE'		=> $user->lang['FIELD_' . strtoupper($gf->profile_types[$row['field_type']])],
		...
		$s_select_type = '';
		foreach ($gf->profile_types as $key => $value)
		{
	...
	*/
	function build_language_options(&$gf, $field_type, $action = 'create')
	{
		...
		foreach ($options as $field => $field_type)
		{
			...
			$lang_options[1]['fields'][$field] = array(
				...
				'FIELD'		=> '<dd>' . ((is_array($gf->vars[$field])) ? implode('<br />', $gf->vars[$field]) : str_replace("\n", '<br />', $gf->vars[$field])) . '</dd>'
			);
		...
		foreach ($languages as $lang_id => $lang_iso)
		{
			...
			foreach ($options as $field => $field_type)
			{
				$value = ($action == 'create') ? request_var('l_' . $field, array(0 => ''), true) : $gf->vars['l_' . $field];

				if ($field == 'lang_options')
				{
					$var = ($action == 'create' || !is_array($gf->vars['l_lang_options'][$lang_id])) ? $gf->vars['lang_options'] : $gf->vars['lang_options'][$lang_id];
				...
				{
					$var = ($action == 'create' || !is_array($gf->vars[$field])) ? $gf->vars[$field] : $gf->vars[$field][$lang_id];
	...
	* Save Profile Field
	*/
	function save_profile_field(&$gf, $field_type, $action = 'create')
	{
		...
		$profile_fields = array(
			'field_length'			=> $gf->vars['field_length'],
			'field_minlen'			=> $gf->vars['field_minlen'],
			'field_maxlen'			=> $gf->vars['field_maxlen'],
			'field_novalue'			=> $gf->vars['field_novalue'],
			'field_default_value'	=> $gf->vars['field_default_value'],
			'field_validation'		=> $gf->vars['field_validation'],
			'field_required'		=> $gf->vars['field_required'],
			'field_show_on_reg'		=> $gf->vars['field_show_on_reg'],
			'field_hide'			=> $gf->vars['field_hide'],
			'field_no_view'			=> $gf->vars['field_no_view']
		);
		...
		if ($action == 'create')
		{
			...
			$sql = 'INSERT INTO ' . GARAGE_CUSTOM_FIELDS_TABLE . ' ' . $db->sql_build_array('INSERT', $profile_fields);
			$db->sql_query($sql);
		...
		{
			$this->update_insert(GARAGE_FIELDS_LANG_TABLE, $sql_ary, array('field_id' => $field_id, 'lang_id' => $default_lang_id));
		}

		if (is_array($gf->vars['l_lang_name']) && sizeof($gf->vars['l_lang_name']))
		{
			foreach ($gf->vars['l_lang_name'] as $lang_id => $data)
			{
				if (($gf->vars['lang_name'] != '' && $gf->vars['l_lang_name'][$lang_id] == '')
					|| ($gf->vars['lang_explain'] != '' && $gf->vars['l_lang_explain'][$lang_id] == '')
					|| ($gf->vars['lang_default_value'] != '' && $gf->vars['l_lang_default_value'][$lang_id] == ''))
				{
				...
				if (!isset($empty_lang[$lang_id]))
				{
					$profile_lang[] = array(
						'field_id'		=> $field_id,
						'lang_id'		=> $lang_id,
						'lang_name'		=> $gf->vars['l_lang_name'][$lang_id],
						'lang_explain'	=> (isset($gf->vars['l_lang_explain'][$lang_id])) ? $gf->vars['l_lang_explain'][$lang_id] : '',
						'lang_default_value'	=> (isset($gf->vars['l_lang_default_value'][$lang_id])) ? $gf->vars['l_lang_default_value'][$lang_id] : ''
					);
				}
			}
		...
		$gf->vars['l_lang_name']			= request_var('l_lang_name', array(0 => ''), true);
		$gf->vars['l_lang_explain']			= request_var('l_lang_explain', array(0 => ''), true);
		$gf->vars['l_lang_default_value']	= request_var('l_lang_default_value', array(0 => ''), true);
		$gf->vars['l_lang_options']			= request_var('l_lang_options', array(0 => ''), true);

		if ($gf->vars['lang_options'])
		{
			if (!is_array($gf->vars['lang_options']))
			{
				$gf->vars['lang_options'] = explode("\n", $gf->vars['lang_options']);
			}
			...
			foreach ($gf->vars['lang_options'] as $option_id => $value)
			{
				$sql_ary = array(
		...
		if (is_array($gf->vars['l_lang_options']) && sizeof($gf->vars['l_lang_options']))
		{
			$empty_lang = array();

			foreach ($gf->vars['l_lang_options'] as $lang_id => $lang_ary)
			{
				...
				if (sizeof($lang_ary) != sizeof($gf->vars['lang_options']))
				{
					$empty_lang[$lang_id] = true;
				}
		...
		foreach ($profile_lang as $sql)
		{
			if ($action == 'create')
			{
				$profile_sql[] = 'INSERT INTO ' . GARAGE_FIELDS_LANG_TABLE . ' ' . $db->sql_build_array('INSERT', $sql);
			}
			else
			{
				$lang_id = $sql['lang_id'];
				...
				$this->update_insert(GARAGE_FIELDS_LANG_TABLE, $sql, array('lang_id' => (int) $lang_id, 'field_id' => $field_id));
			}
		}
		...
		if (sizeof($profile_lang_fields))
		{
			foreach ($profile_lang_fields as $sql)
			{
				if ($action == 'create')
				{
					$profile_sql[] = 'INSERT INTO ' . GARAGE_CUSTOM_FIELDS_LANG_TABLE . ' ' . $db->sql_build_array('INSERT', $sql);
				}
				else
				{
					$lang_id = $sql['lang_id'];
					...
					$this->update_insert(GARAGE_CUSTOM_FIELDS_LANG_TABLE, $sql, array(
						'lang_id'	=> $lang_id, 
						'field_id'	=> $field_id,
						'option_id'	=> $option_id)
					);
		...
		if ($action == 'edit')
		{
			add_log('admin', 'LOG_PROFILE_FIELD_EDIT', $gf->vars['field_ident'] . ':' . $gf->vars['lang_name']);
			trigger_error($user->lang['CHANGED_PROFILE_FIELD'] . adm_back_link($this->u_action));
		}
		else
		{
			add_log('admin', 'LOG_PROFILE_FIELD_CREATE', substr($field_ident, 3) . ':' . $gf->vars['lang_name']);
	...
	function add_field_ident($field_ident, $field_type)
	{
		...
		switch ($db->sql_layer)
		{
			...
			case 'mysqli':
				...
				$sql = 'ALTER TABLE ' . GARAGE_CUSTOM_FIELDS_DATA_TABLE . " ADD `$field_ident` ";
		...
		//						ADD {$field_ident}_bbcode_bitfield INT(11) UNSIGNED";
				...
				{
					$sql = 'ALTER TABLE ' . GARAGE_CUSTOM_FIELDS_DATA_TABLE . " ADD $field_ident [$type]";
				}
		...
		//						ADD {$field_ident}_bbcode_bitfield [INT] UNSIGNED";
			...
			case 'postgres':
				...
				// We are defining the biggest common value, because of the possibility to edit the min/max values of each field.
				$sql = 'ALTER TABLE ' . GARAGE_CUSTOM_FIELDS_DATA_TABLE . " ADD COLUMN \"$field_ident\" ";
