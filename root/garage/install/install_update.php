...
if (!empty($setmodules))
{
	...
	$module[] = array(
		...
		'module_order'		=> 30,
		'module_subs'		=> '',
		'module_stages'		=> array('INTRO', 'VERSION_CHECK', 'UPDATE_DB', 'FILE_CHECK', 'UPDATE_FILES', 'FINAL'),
		'module_reqs'		=> ''
	);
}
...
class install_update extends module
{
	...
	{
		global $template, $phpEx, $phpbb_root_path, $user, $db, $config, $cache, $auth, $garage_config, $config;

		$this->tpl_name = 'garage_install_update';
		$this->page_title = 'UPDATE_INSTALLATION';
		$this->unequal_version = false;

		$this->old_location = $phpbb_root_path . 'garage/install/update/old/';
		$this->new_location = $phpbb_root_path . 'garage/install/update/new/';
		...
		if ($sub == 'intro')
		{
			$cache->destroy('_garage_version_info');
		}
		...
		if (($latest_version = $cache->get('_garage_version_info')) === false)
		{
			$this->latest_version = $this->get_file('version_info');
			$cache->put('_garage_version_info', $this->latest_version);
		}
		else
		{
		...
		$this->current_version = (!empty($garage_config['version_update_from'])) ? $garage_config['version_update_from'] : $garage_config['version'];
		...
		if (!file_exists($phpbb_root_path . 'garage/install/update') || !file_exists($phpbb_root_path . 'garage/install/update/index.' . $phpEx) || !file_exists($this->old_location) || !file_exists($this->new_location))
		{
		...
		if (version_compare(str_replace('rc', 'RC', strtolower($this->current_version)), str_replace('rc', 'RC', strtolower($this->update_info['version']['to'])), '>'))
		{
			$template->assign_vars(array(
				'S_ERROR'		=> true,
				'ERROR_MSG'		=> sprintf($user->lang['INCOMPATIBLE_UPDATE_FILES'], $garage_config['version'], $this->update_info['version']['from'], $this->update_info['version']['to']))
			);
		...
		if ($this->test_update === false)
		{
			...
			{
				$this->tpl_name = '../../garage/install/update/new/adm/style/install_update';
			}

			// What about the language file? Got it updated?
			if (in_array('language/en/garage_install.php', $this->update_info['files']))
			{
				$lang = array();
				include($this->new_location . 'language/en/garage_install.php');
		...
		switch ($sub)
		{
			...
			case 'update_db':
				...
				if (file_exists($phpbb_root_path . 'garage/install/database_update.' . $phpEx))
				{
					include_once($phpbb_root_path . 'garage/install/database_update.' . $phpEx);
				...
				$template->assign_vars(array(
					'S_DB_UPDATE'		=> true,
					'S_DB_UPDATE_FINISHED'	=> ($garage_config['version'] == $this->update_info['version']['to']) ? true : false,
					'U_DB_UPDATE'		=> append_sid($phpbb_root_path . 'garage/install/database_update.' . $phpEx, 'type=1&amp;language=' . $user->data['user_lang']),
					'U_DB_UPDATE_ACTION'	=> append_sid($this->p_master->module_url, "mode=$mode&amp;sub=update_db"),
					'U_ACTION'		=> append_sid($this->p_master->module_url, "mode=$mode&amp;sub=file_check"),
				));
		...
		switch ($sub)
		{
			case 'final':
				...
				$template->assign_vars(array(
					...
					'BODY'		=> sprintf($user->lang['UPDATE_CONGRATS_EXPLAIN'], $garage_config['version'], append_sid($phpbb_root_path . 'install/index.' . $phpEx, 'mode=convert&amp;'), '../docs/README.html'),
				));
