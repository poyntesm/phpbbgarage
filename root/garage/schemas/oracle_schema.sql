...
CREATE TABLE phpbb_dynoruns_gallery (
	...
	image_id number(8) DEFAULT '0' NOT NULL,
	hilite number(1) DEFAULT '0' NOT NULL,
	CONSTRAINT pk_phpbb_dynoruns_gallery PRIMARY KEY (id)
)
/
...
CREATE SEQUENCE phpbb_dynoruns_gallery_seq
/

CREATE OR REPLACE TRIGGER t_phpbb_dynoruns_gallery
BEFORE INSERT ON phpbb_dynoruns_gallery
FOR EACH ROW WHEN (
	new.id IS NULL OR new.id = 0
)
BEGIN
	SELECT phpbb_dynoruns_gallery_seq.nextval
	INTO :new.id
	FROM dual;
END;
/
